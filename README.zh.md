...
如果源术语出现在任何翻译段中，输出必须包含相应的目标术语。当规则检查失败时，工具会重新提示模型以包含缺失的术语。重试预算由 `retry_times` 控制。如果多次失败，工具会调用术语判定提示判断该翻译是否仍可接受；若接受则直接采信，否则继续重试。若超过重试次数，会采用最后一次翻译结果作为兜底。
